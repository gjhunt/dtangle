...
test_that("basic dtangle2 Y and refs and ps", {
    ...
    dt_out <- dtangle2(Y, references = references, pure_samples = pure_samples, n_markers = n_markers, 
...
test_that("basic dtangle2 Y and refs", {
    ...
    dt_out <- dtangle2(Y, references = combined_refs, n_markers = n_markers, markers = comp$markers, 
...
test_that("basic dtangle2 Y and pure_samples", {
    ...
    n_markers <- 20
    comp <- readRDS("basic_dtangle2.rds")
    dt_out <- dtangle2(Y, pure_samples = pure_samples, n_markers = n_markers, markers = comp$markers, 
        seed = 4261992, dtangle_init = FALSE)
    expect_equal(dt_out[-5], comp[-5], tolerance = 1e-05)
...
test_that("basic dtangle2 markers", {
    Y <- shen_orr_ex$data$log
    n_markers <- 20
    dt_out <- dtangle2(Y, pure_samples = pure_samples, n_markers = c(10, 11, 12), 
...
test_that("basic dtangle2 dtangle_init", {
    Y <- shen_orr_ex$data$log
    n_markers <- 20
    dt_out <- dtangle2(Y, pure_samples = pure_samples, n_markers = n_markers, marker_method = "ratio", 
        seed = 4261992, dtangle_init = FALSE)
    expect_equal_to_reference(dt_out, "basic_dtangle2.rds")
    dt_out <- dtangle2(Y, pure_samples = pure_samples, n_markers = n_markers, marker_method = "ratio", 
        seed = 4261992, dtangle_init = TRUE)
...
test_that("basic dtangle2 variance weights", {
    ...
    n_markers <- 20
    dt_out <- dtangle2(Y, pure_samples = pure_samples, n_markers = n_markers, seed = 4261992, 
        dtangle_init = FALSE, weights = "variance")
...
test_that("basic dtangle2 arbitrary weights", {
    ...
    wts <- 1:ncol(Y)
    dt_out <- dtangle2(Y, pure_samples = pure_samples, n_markers = n_markers, seed = 4261992, 
        dtangle_init = FALSE, weights = wts)
