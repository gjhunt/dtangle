...
test_that("basic_deconv", {
    dt_out <- dtangle(Y, pure_samples = pure_samples, n_markers = n_markers, data_type = "microarray-gene", 
        marker_method = "ratio")
...
test_that("basic deconv Y, refs, pure_samples", {
    ...
    dt_out <- dtangle(Y, references = references, pure_samples = pure_samples, n_markers = n_markers, 
...
test_that("basic deconv n_markers", {
    ...
    dt_out <- dtangle(Y, references = combined_refs, n_markers = c(10, 11, 12), data_type = "microarray-gene", 
        markers = comp$markers)
