...
gen_sim2 <- function(n_samples = 50, mu = 1, sigma = 1, seed = NULL, dset = parsons, 
    ...
    medv = median(log2(1 + Z))
    wgs = colnames(Z)[head(order(abs(Z[1, ] - medv), decreasing = FALSE), n = 60)]
    Z[1, wgs[1:20]] <- medv * 2
    Z[2, wgs[21:40]] <- medv * 2
    Z[3, wgs[41:60]] <- medv * 2
    Z[, !(colnames(Z) %in% wgs)] <- Z[, !(colnames(Z) %in% wgs)] + array(runif(prod(dim(Z[, 
        !(colnames(Z) %in% wgs)])), -0.01, 0.01), dim(Z[, !(colnames(Z) %in% wgs)]))
