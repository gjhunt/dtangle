...
#-------------------------------------------------------------------#
# initial estimate of cell type composition yr = c(tumor purity, expression of nG
# genes) Z is the expression signature matrix
#-------------------------------------------------------------------#
...
sigmaInit <- function(x, Z, nG, var_wgt) {
    ...
    # g2use is the initial set of abberant genes. It represents the first look at
    # aberrant genes by selecting them from the the upper quartile of residuals from
    # initial fit (worst fit 25%).  The remaining 75% are considered 'consistent
    # marker genes'.
    
    log_eta_ij = log(eta_ij)
    init_val[1] = sigma2_Update(logY = logY[-c(g2use)], log_eta_ij = log_eta_ij[-c(g2use)], 
        EM_wgt = rep(1, (nG - length(g2use))), AB_Up = FALSE, var_wgt = var_wgt)
    
    init_val[2] = sigma2_Update(logY = logY[c(g2use)], log_eta_ij = log_eta_ij[c(g2use)], 
...
sigma2_obj_function <- function(x, logY, log_eta_ij, var_wgt, EM_wgt, AB_Up = FALSE) {
    ...
    if (AB_Up == FALSE) {
        ...
        out = sum((1 - EM_wgt) * dnorm(logY, mean = mu_ij, sd = sqrt(sigma2_wgt), 
            log = TRUE))
    }
    return(out)
}

sigma2_Update <- function(logY, log_eta_ij, EM_wgt, AB_Up = FALSE, var_wgt = NULL) {
    
    if (is.null(var_wgt)) {
        varTheta_ij = logY - log_eta_ij
        
        if (AB_Up == FALSE) {
            Sigma2_Up = 2 * (sqrt((sum(EM_wgt * (varTheta_ij^2))/sum(EM_wgt)) + 1) - 
                1)
        } else {
            Sigma2_Up = 2 * (sqrt((sum((1 - EM_wgt) * (varTheta_ij^2))/sum(1 - EM_wgt)) + 
                1) - 1)
        }
    } else {
        optimizeOut = optimize(f = sigma2_obj_function, logY = logY, log_eta_ij = log_eta_ij, 
            var_wgt = var_wgt, AB_Up = AB_Up, EM_wgt = EM_wgt, lower = 0, upper = 10, 
            maximum = TRUE)
...
extractFunction <- function(compList, element) {
    out = mapply(compList, FUN = function(x) {
        get(element, x)
    })
    return(out)
}
...
gradFunc_noPurity <- function(x, logY, Z, sigma2C, sigma2A, EM_wgt) {
    ...
    c1 = c(d_ijC * EM_wgt/(sigma2C * eta_ij)) + c(d_ijA * (1 - EM_wgt)/(sigma2A * 
        eta_ij))
...
gradFunc_givenPurity <- function(x, logY, rho_i0, Z, sigma2C, sigma2A, EM_wgt) {
    ...
    # the derivative was derived using all cell type composition of non-tumor cell
    # types. Since the proportion of last cell type is rho_K = constant - sum(x), its
    # derivative is the original derivative multiples d [rho_K] / d [rho_k], for k =
    # 1 to K-1, which is - 1 so it is equivalent to replace Z by Z_star
    nCT = ncol(Z)
...
# hin and hin_jacob are needed for Augmented Lagrangian Minimization Algorithm
# for optimization hin: a vector function specifying inequality constraints such
# that hin[j] > 0 for all j hin.jac: Jacobian of hin
...
updatePropn_Single <- function(x, Z, nCT, nG, maxIter_prop, givenPurity, var_wgt) {
    #----------------------------------------#
    # Extract Info #
    #----------------------------------------#
    rho_i0 = x[1]
    ...
    sigma2C_1 = sigma2_Update(logY = logY, log_eta_ij = log_eta_ij, EM_wgt = EM_wgt, 
        AB_Up = FALSE, var_wgt = var_wgt)
    sigma2A_1 = sigma2_Update(logY = logY, log_eta_ij = log_eta_ij, EM_wgt = EM_wgt, 
    ...
    for (k in 1:maxIter_prop) {
        ...
        #----------------------------------#
        if (givenPurity) {
            auglagOut = auglag(par = urho_0[-c(nCT)], fn = logLik_givenPurity, gr = gradFunc_givenPurity, 
                hin = hin_func_givenPurity, hin.jac = hin_jacob_givenPurity, logY = logY, 
                rho_i0 = rho_i0, Z = Z, sigma2C = sigma2C_0, sigma2A = sigma2A_0, 
                EM_wgt = EM_wgt, control.optim = list(fnscale = -1), control.outer = list(trace = FALSE))
            ...
            auglagOut = auglag(par = urho_0, fn = logLik_noPurity, gr = gradFunc_noPurity, 
                hin = hin_func_noPurity, hin.jac = hin_jacob_noPurity, logY = logY, 
                Z = Z, sigma2C = sigma2C_0, sigma2A = sigma2A_0, EM_wgt = EM_wgt, 
        ...
        eta_ij = drop(Z %*% matrix(urho_1, ncol = 1))
        log_eta_ij = log(eta_ij)
        sigma2C_1s = sigma2_Update(logY = logY, log_eta_ij = log_eta_ij, EM_wgt = EM_wgt, 
            AB_Up = FALSE, var_wgt = var_wgt)
        sigma2A_1s = sigma2_Update(logY = logY, log_eta_ij = log_eta_ij, EM_wgt = EM_wgt, 
            AB_Up = TRUE, var_wgt = var_wgt)
        ...
        logLik_1 = sum(EM_wgt * dnorm(logY, mean = mu_ijC, sd = sqrt(sigma2C_1), 
            log = TRUE)) + sum((1 - EM_wgt) * dnorm(logY, mean = mu_ijA, sd = sqrt(sigma2A_1), 
            log = TRUE))
        
        # if(logLik_1 < logLik_0 - 0.001){ stop('log likelihood decreased during the EM
        # algorithm.') }
        
        if (max(abs(urho_1 - urho_0)) < 0.001) {
            break
        }
    }
...
updatePropn_All <- function(logY, rho, tumorPurity, Z, maxIter_prop, EM_wgt, givenPurity, 
    var_wgt) {
    
    PropnInfo = rbind(tumorPurity, rho, logY, EM_wgt)
    
    out = apply(X = PropnInfo, MARGIN = 2, FUN = updatePropn_Single, Z = Z, givenPurity = givenPurity, 
        maxIter_prop = maxIter_prop, nCT = (nrow(rho)), nG = nrow(logY), var_wgt = var_wgt)
    ...
    return(list(rho_Curr = rho_Mat, sig2C_Curr = sig2C_Mat, sig2A_Curr = sig2A_Mat))
}
...
PropPlus_Update <- function(Y, rho_0, tumorPurity, givenPurity, sigma2C_0, sigma2A_0, 
    Z, propC_0, maxIter_PP, maxIter_prop, nG, rhoConverge, var_wgt) {
    logY = log(Y)
    ...
    for (j in 1:maxIter_PP) {
        ...
        # Update EM Weights
        EM_wgt = updateWgts(logY = logY, rho = rho_t0, sigma2C = sigma2C_t0, sigma2A = sigma2A_t0, 
            Z = Z, propC = propC_t0, var_wgt = var_wgt)
        
        # Update Proportions
        PropP_t1 = updatePropn_All(logY = logY, rho = rho_t0, tumorPurity = tumorPurity, 
            Z = Z, maxIter_prop = maxIter_prop, EM_wgt = EM_wgt, givenPurity = givenPurity, 
            var_wgt = var_wgt)
    ...
    return(list(rho = rho_t1, sigma2C = sigma2C_t1, sigma2A = sigma2A_t1, propC = propC_t1, 
        Iter = j, max_rho_diff = max_rho_diff))
}
...
refEstimate <- function(X, borrow4SD = TRUE) {
    ...
    CT_MU = t(apply(X = logX, MARGIN = 1, FUN = meanFun, group = cellType))
    
    CT_var = t(apply(X = logX, MARGIN = 1, FUN = varFun, group = cellType))
...
ICeDT <- function(Y, Z, tumorPurity = NULL, refVar = NULL, rhoInit = NULL, maxIter_prop = 100, 
    maxIter_PP = 100, rhoConverge = 0.001) {
    ...
    sigma_0 = apply(X = rbind(Y, rho_0), MARGIN = 2, FUN = sigmaInit, Z = Z, nG = nG, 
        var_wgt = var_wgt)
    ...
    # EM algorithm #
    #-----------------------------------------------------#
    PropPlus_Out = suppressWarnings(PropPlus_Update(Y = Y, rho_0 = rho_0, tumorPurity = tumorPurity, 
        givenPurity = givenPurity, sigma2C_0 = sigma2C_0, sigma2A_0 = sigma2A_0, 
        Z = Z, propC_0 = propC_0, maxIter_PP = maxIter_PP, maxIter_prop = maxIter_prop, 
    ...
    EM_wgt = updateWgts(logY = log(Y), rho = rho_1, sigma2C = sigma2C_1, sigma2A = sigma2A_1, 
    ...
    outList = list(rho = rho_final, sigma2C = sigma2C_1, sigma2A = sigma2A_1, cProp = propC_1, 
        cProb = EM_wgt)
