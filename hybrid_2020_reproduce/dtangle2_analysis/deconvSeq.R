getx1.rnaseq = function(resultb0, dge_s, MAXITER = 1000, x0 = NULL) {
    # if (NB0 == 'all') { NB0 = length(resultb0$pfstat) }else if (NB0 ==
    # 'top_bonferroni') { NB0 = length(which(sort(p.adjust(resultb0$pfstat,
    # 'bonferroni')) < 0.05)) }else if (NB0 == 'top_fdr') { NB0 =
    # length(which(sort(p.adjust(resultb0$pfstat, 'fdr')) < 0.05)) }
    b0r.1 = resultb0  #$b0
    # pfstat.1 = resultb0$pfstat
    b0r = b0r.1[which(rownames(b0r.1) %in% dge_s$genes[, 1]), ]
    # pfstat = pfstat.1[which(rownames(b0r.1) %in% dge_s$genes[, 1])] b0r =
    # b0r[order(pfstat)[1:NB0], ]
    ntypes = ncol(b0r)
    if (is.null(x0)) 
    ...
    for (i in 1:nsamples_s) {
        ...
        opt_result = tryCatch(solnp(pars = x0, fun = obj, eqfun = gx, eqB = 0, LB = rep(0, 
            ntypes), UB = rep(1, ntypes), control = list(outer.iter = MAXITER, inner.iter = MAXITER, 
